...
public class LogEventConvert extends AbstractCanalLifeCycle implements BinlogParser<LogEvent> {
    ...
    private boolean processFilter(String queryString, DdlResult result){
        ...
        // fixed issue https://github.com/alibaba/canal/issues/58
        if(result.getType() == EventType.ALTER || result.getType() == EventType.ERASE || result.getType() == EventType.CREATE || result.getType() == EventType.TRUNCATE || result.getType() == EventType.RENAME || result.getType() == EventType.CINDEX || result.getType() == EventType.DINDEX){ // 针对DDL类型
    ...
    private EntryPosition createPosition(LogHeader logHeader){
        return new EntryPosition(binlogFileName, logHeader.getLogPos(), logHeader.getWhen() * 1000L, logHeader.getServerId()); // 记录到秒
    }
